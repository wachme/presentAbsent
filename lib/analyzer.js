...
Analyzer.prototype.average = function(period) {
    ...
    var periods = [];
    var days = this.days;
    function average(period, lesson) {
        if (lesson == null) {
            return;
        }
        if (!period.averages[lesson.lesson.name]) {
            period.averages[lesson.lesson.name] = {
                all : 0,
                present : 0
            }
        }

        with (period.averages[lesson.lesson.name]) {
            present = (present * all + (lesson.present === true)) / (++all)
        }
    }
    ...
    for (var i = 0; i < days.length; i++) {
        ...
        var period = periods[Math.floor(i / p)];
        for (var j = 0; j < day.lessons.length; j++) {
            var lesson = day.lessons[j];
            average(period, lesson);
            if (lesson) {
                average(period, {
                    lesson : {
                        name : 'overall'
